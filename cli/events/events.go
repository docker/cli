...
// Options specifies common options to use for system events.
type Options struct {
	Type   string
	Since  string
	Until  string
...
// NewObjectEventsCommand creates a new cobra.Command for the given object-type.
func NewObjectEventsCommand(dockerCli command.Cli, objectType string) *cobra.Command {
	options := Options{
		Type:   objectType,
	...
	cmd := &cobra.Command{
		Use:   "events [OPTIONS]",
		Short: "Get real time " + options.Type + " events from the server",
		Args:  cli.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			return Run(dockerCli, &options)
		},
	}

	flags := cmd.Flags()
	InstallFlags(flags, &options)
	return cmd
}

// InstallFlags installs common flags for system events.
func InstallFlags(flags *pflag.FlagSet, options *Options) {
...
// Run runs the events command
func Run(dockerCli command.Cli, options *Options) error {
	if options.Type != "" {
		if options.Filter.Value().Contains("type") {
			return cli.StatusError{
				StatusCode: 64,
				Status:     "The --filter type=<object-type> option is not supported when viewing " + options.Type + " events.",
			}
		}
		options.Filter.Value().Add("type", options.Type)
	}
