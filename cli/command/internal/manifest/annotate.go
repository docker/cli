...
func newManifestStore(dockerCLI cli.Cli) store.Store {
...
// registry
func newRegistryClient(dockerCLI cli.Cli, allowInsecure bool) registryclient.RegistryClient {
	if msp, ok := dockerCLI.(manifestStoreProvider); ok {
		// manifestStoreProvider is used in tests to provide a dummy store.
...
func newAnnotateCommand(dockerCli cli.Cli) *cobra.Command {
...
func runManifestAnnotate(dockerCLI cli.Cli, opts annotateOptions) error {
