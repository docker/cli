...
func init() {
	commands.RegisterCommand(newAttachCommand)
}
...
func NewAttachCommand(dockerCli cli.Cli) *cobra.Command {
	return newAttachCommand(dockerCli)
}

// newAttachCommand creates a new cobra.Command for `docker attach`
var newAttachCommand = commands.MaybeHideLegacy(func(dockerCLI cli.Cli) *cobra.Command {
...
//
// deprecated: commands should never directly get imported
func RunAttach(ctx context.Context, dockerCLI cli.Cli, containerID string, opts *AttachOptions) error {
...
func resizeTTY(ctx context.Context, dockerCli cli.Cli, containerID string) {
