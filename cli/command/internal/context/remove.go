...
func RunRemove(dockerCLI cli.Cli, opts RemoveOptions, names []string) error {
	var errs []error
...
func doRemove(dockerCli cli.Cli, name string, isCurrent, force bool) error {
	if isCurrent {
		if !force {
...
func checkContextExists(dockerCli cli.Cli, name string) error {
