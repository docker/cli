...
func TestConfigInspectWithoutFormat(t *testing.T) {
	...
	for _, tc := range testCases {
		fakeCli := test.NewFakeCli(&fakeClient{configInspectFunc: tc.configInspectFunc})
		cmd := newConfigInspectCommand(fakeCli)
		cmd.SetArgs(tc.args)
		assert.NilError(t, cmd.Execute())
		golden.Assert(t, fakeCli.OutBuffer().String(), fmt.Sprintf("config-inspect-without-format.%s.golden", tc.name))
	}
}
...
func TestConfigInspectWithFormat(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		assert.NilError(t, cmd.Execute())
		golden.Assert(t, fakeCli.OutBuffer().String(), fmt.Sprintf("config-inspect-with-format.%s.golden", tc.name))
	}
}
...
func TestConfigInspectPretty(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		assert.NilError(t, cmd.Execute())
		golden.Assert(t, fakeCli.OutBuffer().String(), fmt.Sprintf("config-inspect-pretty.%s.golden", tc.name))
	}
}
