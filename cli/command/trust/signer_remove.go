...
func removeSigner(ctx context.Context, cli command.Cli, options signerRemoveOptions) error {
	var errRepos []string
	for _, repo := range options.repos {
		fmt.Fprintf(cli.Out(), "Removing signer \"%s\" from %s...\n", options.signer, repo)
		if _, err := removeSingleSigner(ctx, cli, repo, options.signer, options.forceYes); err != nil {
...
func removeSingleSigner(ctx context.Context, cli command.Cli, repoName, signerName string, forceYes bool) (bool, error) {
