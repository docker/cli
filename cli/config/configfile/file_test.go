...
func TestConfigEnv(t *testing.T) {
	t.Run("case=should error on unexpected fields", func(t *testing.T) {
		c := configEnv{}
		assert.ErrorContains(t, c.LoadFromEnv(envTestUserPassConfig), "json: unknown field \"username\"")
	})
	t.Run("case=should be able to load DOCKER_AUTH_CONFIG", func(t *testing.T) {
		c := configEnv{}
		assert.NilError(t, c.LoadFromEnv(envTestAuthConfig))
		expected := configEnv{
			AuthConfigs: map[string]configEnvAuth{
				"env.example.test": {
					Auth: "ZW52X3VzZXI6ZW52X3Bhc3M=",
				},
			},
		}
		assert.Check(t, is.DeepEqual(c, expected))
	})
}
