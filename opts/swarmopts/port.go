...
func ConvertPortToPortConfig(
	...
	portProto.Num()
	proto, portInt := portProto.Proto(), portProto.Num()
	...
	for _, binding := range portBindings[nat.Port(portProto.String())] {
		if p := net.ParseIP(binding.HostIP); p != nil && !p.IsUnspecified() {
			...
			logrus.Warnf("ignoring IP-address (%s:%s) service will listen on '0.0.0.0'", net.JoinHostPort(binding.HostIP, binding.HostPort), portProto)
		}
