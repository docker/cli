...
func ConvertPortToPortConfig(
	...
	proto, portInt := portProto.Proto(), portProto.Int()

	ports := make([]swarm.PortConfig, 0, len(portBindings))
	for _, binding := range portBindings[portProto] {
		if !binding.HostIP.IsUnspecified() {
			// TODO(thaJeztah): use context-logger, so that this output can be suppressed (in tests).
			logrus.Warnf("ignoring IP-address (%s:%s) service will listen on '0.0.0.0'", net.JoinHostPort(binding.HostIP.String(), binding.HostPort), portProto)
		}

		startHostPort, endHostPort, err := nat.ParsePortRange(binding.HostPort)

		if err != nil && binding.HostPort != "" {
			return nil, fmt.Errorf("invalid hostport binding (%s) for port (%d)", binding.HostPort, portProto.Int())
		}
