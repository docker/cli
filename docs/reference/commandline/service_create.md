...
as numerical IDs or names. The `mode` is specified as a 4-number sequence such
as `0755`. If a `umask` is set in the container, it impacts the `mode` you set.
...
the container. If a target is specified, that is the filename.
