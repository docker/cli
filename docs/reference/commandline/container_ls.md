...
Running `docker container ls --no-trunc` showing 2 linked containers.
...
The `docker container ls` command only shows running containers by default. To see all
...
`docker container ls` groups exposed ports into a single range if possible. E.g., a
container that exposes TCP ports `100, 101, 102` displays `100-102/tcp` in
the `PORTS` column.

### Show disk usage by container

The `docker container ls -s` command displays two different on-disk-sizes for each container:
...
The `label` filter matches containers based on the presence of a `label` alone
or a `label` and a value.

The following filter matches containers with the `color` label regardless of its value.

```bash
$ docker container ls --filter "label=color"
...
```bash
$ docker container ls --filter "label=color=blue"
...
```bash
$ docker container ls --filter "name=nostalgic_stallman"
...
```bash
$ docker container ls --filter "name=nostalgic"
...
```bash
$ docker container ls -a --filter 'exited=137'
...
```bash
$ docker container ls --filter ancestor=ubuntu
...
```bash
$ docker container ls --filter ancestor=ubuntu-c1
...
```bash
$ docker container ls --filter ancestor=ubuntu:12.04.5
...
```bash
$ docker container ls --filter ancestor=d0e008c6cf02
...
```bash
$ docker container ls -f before=9c3527ed70ce
...
```bash
$ docker container ls -f since=6e63f6ff38b0
...
$ docker container ls --filter network=net1
...
$ docker container ls --filter network=8c0b4110ae930dbe26b258de9bc34a03f98056ed6f27f991d32919bfe401d7c5
...
The `publish` and `expose` filters show only containers that have published or
exposed port with a given port number, port range, and/or protocol. The default
...
$ docker container ls --filter publish=80
...
```bash
$ docker container ls --filter expose=8000-8080/tcp
