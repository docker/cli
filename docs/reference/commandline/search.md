...
The filtering flag (`-f` or `--filter`) format is a `key=value` pair. The supported key and value given below. If there is more
than one filter, then pass multiple flags (e.g. `--filter is-automated=true --filter stars=3`)

The currently supported filters(key=value) are:
