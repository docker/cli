...
| [StorageOS plugin](https://storageos.com/install)                                                | A volume plugin which turns your Docker node into a hyper-converged storage platform. Each Docker host can contribute available local or attached storage into a distributed pool, which is then available to all cluster members via a global namespace.|
